...
from numpy import arange, loadtxt, savetxt, array
...
class ginSetup(object):
    ...
    def query_compute_capability(self):
        ...
        for devicenum in range(cuda.Device.count()):
            device = cuda.Device(devicenum)
            attrs = device.get_attributes()
            return "%d%d" % (attrs[pycuda._driver.device_attribute.COMPUTE_CAPABILITY_MAJOR],
    ...
    def check_last_access(self, saved_lastaccess):
        ...
        if saved_lastaccess < read_lastaccess:
            print(
                " * The file '__jitkernel.cu' changed since last access, ginSODA must re-build the whole binary executable file.")
    ...
    def compile(self, path_main, path_ginsoda, JUST_LINK=False, COMPILE_2B=False, verbose=True):
        ...
        if COMPILE_2B:
            ...
            command1 = [self.cuda_compiler, "--keep-dir", self.temp_dir_objects, "-maxrregcount=0", "--machine",
                        str(self.machine_BITS), "--compile", "-Wno-deprecated-gpu-targets",
                        "--compiler-options", OPTIMIZATION_FLAGS, "-o",
                        self.temp_dir_objects + "/" + path_ginsoda + ".obj", path_ginsoda, "-clean", "-w"]
            ...
            command3 = [self.cuda_compiler, "-arch=sm_" + self._architecture,
                        "--keep-dir", self.temp_dir_objects, "-maxrregcount=0", "--machine", str(self.machine_BITS),
                        "--compile", "-cudart", "static", "-use_fast_math", "-Wno-deprecated-gpu-targets",
                        "--compiler-options", OPTIMIZATION_FLAGS, "-o",
                        self.temp_dir_objects + "/" + path_ginsoda + ".obj", path_ginsoda, "-w"]
            ...
            if not SKIP_COMPILE_1A:
                ...
                command4 = [self.cuda_compiler, "-arch=sm_" + self._architecture,
                            ...
                            "--machine", str(self.machine_BITS), "--compile", "-cudart", "static", "-use_fast_math",
        ...
        if COMPILE_2B:  # that is, just link
            # link process
            commands5 = [self.cuda_compiler, "-arch=sm_" + self._architecture,
                         self.temp_dir_objects + "/" + path_main + ".obj",
    ...
    def run(
        ...
        if len(parameters) != len(initial_values):
            raise Exception(
                "parameterizations and initial states have different lengths (" + str(len(parameters)) + " and " + str(
                    len(initial_values)) + ", respectively)")
        ...
        if len(observed) == 0: observed = arange(
            len(initial_values[0]))  # sample all species if observed are not specified
        ...
        self.create_model_files(parameters=parameters, initial_values=initial_values, time_instants=time_instants,
                                observed=observed)
    ...
    def estimate_memory_requirements(self, THREADS, time_instants):
        ...
        lrw = 22 + len(self.SD.variables) * max(16, len(self.SD.variables) + 9);
        rwork_bytes = THREADS * 8 * lrw
        liw = 20 + len(self.SD.variables);
        iwork_bytes = THREADS * liw * 4
